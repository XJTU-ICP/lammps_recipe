...
cmake -D CMAKE_BUILD_TYPE=Release -D BUILD_LIB=ON -D BUILD_SHARED_LIBS=ON -D LAMMPS_INSTALL_RPATH=ON -DCMAKE_INSTALL_LIBDIR=lib $ARGS -D FFT=FFTW3 -D CMAKE_INSTALL_PREFIX=${PREFIX} ../cmake -DCMAKE_CXX_FLAGS='-D_GLIBCXX_USE_CXX11_ABI=0'  # use -DCMAKE_CXX_FLAGS='-D_GLIBCXX_USE_CXX11_ABI=0' to force lammps not using CXX11_ABI and compatible with pytorch
make -j${NUM_CPUS}
make install
cd ../python
pip install -vv .
